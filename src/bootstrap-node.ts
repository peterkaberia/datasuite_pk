...
import type { IProductConfiguration } from './cd/base/common/product.js';
...
if (!process.env['CDSUITE_HANDLES_SIGPIPE']) {
...
// -  all OS: store the `process.cwd()` inside `CDSUITE_CWD` for consistent lookups
function setupCurrentWorkingDirectory(): void {
	try {

		// Store the `process.cwd()` inside `CDSUITE_CWD`
		// for consistent lookups, but make sure to only
		// do this once unless defined already from e.g.
		// a parent process.
		if (typeof process.env['CDSUITE_CWD'] !== 'string') {
			process.env['CDSUITE_CWD'] = process.cwd();
		}
...
export function configurePortable(product: Partial<IProductConfiguration>): { portableDataPath: string; isPortable: boolean } {
	...
	function getPortableDataPath(): string {
		if (process.env['CDSUITE_PORTABLE']) {
			return process.env['CDSUITE_PORTABLE'];
		}
	...
	if (isPortable) {
		process.env['CDSUITE_PORTABLE'] = portableDataPath;
	} else {
		delete process.env['CDSUITE_PORTABLE'];
	}
