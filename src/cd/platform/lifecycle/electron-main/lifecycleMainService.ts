	...
	*/
	...
	private onBeforeUnloadWindowInRenderer(window: ICodeWindow, reason: UnloadReason): Promise<boolean /* veto */> {
		return new Promise<boolean>(resolve => {
			...
			const okChannel = `cdsuite:ok${oneTimeEventToken}`;
			const cancelChannel = `cdsuite:cancel${oneTimeEventToken}`;
			...
			window.send('cdsuite:onBeforeUnload', { okChannel, cancelChannel, reason });
		});
	}
	...
	private onWillUnloadWindowInRenderer(window: ICodeWindow, reason: UnloadReason): Promise<void> {
		return new Promise<void>(resolve => {
			const oneTimeEventToken = this.oneTimeListenerTokenGenerator++;
			const replyChannel = `cdsuite:reply${oneTimeEventToken}`;

			validatedIpcMain.once(replyChannel, () => resolve());

			window.send('cdsuite:onWillUnload', { replyChannel, reason });
		});
	}
