	...
	protected override createWatcher(disposables: DisposableStore): IUniversalWatcher {
		...
		const client = disposables.add(new Client(
			FileAccess.asFileUri('bootstrap-fork').fsPath,
			{
				...
				env: {
					CDSUITE_ESM_ENTRYPOINT: 'cd/platform/files/node/watcher/watcherMain',
					CDSUITE_PIPE_LOGGING: 'true',
					CDSUITE_VERBOSE_LOGGING: 'true' // transmit console logs from server to client
				}
			}
		));
