...
export class Client implements IChannelClient, IDisposable {
	...
	private get client(): IPCClient {
		if (!this._client) {
			...
			forkOpts.env = { ...deepClone(process.env), 'CDSUITE_PARENT_PID': String(process.pid) };
			...
			if (forkOpts.execArgv === undefined) {
				forkOpts.execArgv = process.execArgv			// if not set, the forked process inherits the execArgv of the parent process
					...
					.filter(a => !a.startsWith('--cdsuite-')); 	// --cdsuite-* arguments are unsupported by node.js and thus need to remove
			}
