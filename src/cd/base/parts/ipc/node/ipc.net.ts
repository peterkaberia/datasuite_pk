...
export function createRandomIPCHandle(): string {
	...
	if (process.platform === 'win32') {
		return `\\\\.\\pipe\\cdsuite-ipc-${randomSuffix}-sock`;
	}
	...
	const result = join(basePath, `cdsuite-ipc-${randomSuffix}.sock`);
...
export function createStaticIPCHandle(directoryPath: string, type: string, version: string): string {
	...
	let result: string;
	if (process.platform !== 'darwin' && XDG_RUNTIME_DIR && !process.env['CDSUITE_PORTABLE']) {
		result = join(XDG_RUNTIME_DIR, `cdsuite-${scopeForSocket}-${versionForSocket}-${typeForSocket}.sock`);
	} else {
