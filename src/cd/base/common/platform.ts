...
if (typeof $globalThis.cdsuite !== 'undefined' && typeof $globalThis.cdsuite.process !== 'undefined') {
	// Native environment (sandboxed)
	nodeProcess = $globalThis.cdsuite.process;
...
if (typeof nodeProcess === 'object') {
	...
	const rawNlsConfig = nodeProcess.env['CDSUITE_NLS_CONFIG'];
	if (rawNlsConfig) {
		try {
...
export const setTimeout0 = (() => {
	if (setTimeout0IsFaster) {
		...
		$globalThis.addEventListener('message', (e: any) => {
			if (e.data && e.data.cdsuiteScheduleAsyncWork) {
				for (let i = 0, len = pending.length; i < len; i++) {
					const candidate = pending[i];
					if (candidate.id === e.data.cdsuiteScheduleAsyncWork) {
						pending.splice(i, 1);
						candidate.callback();
						return;
					}
		...
		return (callback: () => void) => {
			...
			$globalThis.postMessage({ cdsuiteScheduleAsyncWork: myId }, '*');
		};
	}
